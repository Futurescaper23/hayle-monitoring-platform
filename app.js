...
const SURVEY_MODEL_TABS = new Set(["areas", "panorama", "fixedpoints", "volume", "layers", "sections"]);
const trendAreaCachePromises = new Map();

  const PROJECT_SECTION_IMAGE_TRACKS = {
    "legacy-estuary": {
      area1: [
    ...
    "hayle-estuary": {
      area1: [
        { upper: { x: 25.4, y: 38.7 }, lower: { x: 27.9, y: 85.2 } },
        { upper: { x: 49.8, y: 23.6 }, lower: { x: 55.1, y: 70.7 } },
        { upper: { x: 74.8, y: 29.4 }, lower: { x: 76.1, y: 73.0 } }
      ],
      area2: [
        { upper: { x: 26.8, y: 21.2 }, lower: { x: 9.5, y: 41.6 } },
        { upper: { x: 44.7, y: 36.9 }, lower: { x: 25.6, y: 61.9 } },
        { upper: { x: 74.8, y: 43.7 }, lower: { x: 41.3, y: 81.5 } }
      ],
      area3: [
        { upper: { x: 16.9, y: 15.4 }, lower: { x: 18.2, y: 75.4 } },
        { upper: { x: 42.0, y: 3.3 }, lower: { x: 42.9, y: 94.4 } },
        { upper: { x: 67.3, y: 7.7 }, lower: { x: 58.6, y: 95.5 } },
        { upper: { x: 86.0, y: 8.3 }, lower: { x: 86.9, y: 63.9 } }
      ]
    }
  };

function explicitSectionImageTracks(projectId, areaId) {
  return PROJECT_SECTION_IMAGE_TRACKS[projectId]?.[areaId] || null;
}

const PROJECT_DETECTED_SECTION_TRACK_AREAS = {
  "hayle-estuary": new Set(["area3"])
};

function shouldUseDetectedSectionTracks(projectId, areaId) {
  return Boolean(PROJECT_DETECTED_SECTION_TRACK_AREAS[projectId]?.has(areaId));
}

const PROJECT_REVERSED_SECTION_PROFILES = {
  "hayle-estuary": {
    area1: new Set(["A1-01", "A1-02", "A1-03"]),
    area3: new Set(["A3-02", "A3-03", "A3-04"])
  }
};

function sectionProfileShouldBeReversed(projectId, areaId, sectionId) {
  return Boolean(PROJECT_REVERSED_SECTION_PROFILES[projectId]?.[areaId]?.has(sectionId));
}
...
const SECTION_DISPLAY_MIN_HEIGHT = -3;
const SECTION_DISPLAY_MAX_HEIGHT = 5;

const BASELINE_AREA_OVERVIEW = {
  area1: {
    title: "Hawker's Cove / Tregirls Beach",
    zone: "Outer estuary / outer shore",
    day: "Day 1",
    filterKey: "day1",
    ...
    tideScore: 61,
    missionRole: "Covers the outer estuary section and completes the Day 1 survey sequence.",
    ...
    tags: ["Day 1", "Outer shore", "Outer reach", "Late-tide context"],
    cardNote: "Outer estuary baseline",
    purpose: "Shows conditions in the outer estuary section after low tide and helps link this area to the rest of the estuary survey."
  },
  area2: {
    title: "Daymer Bay",
  ...
  area4: {
    title: "Harbour Estuary",
    zone: "Harbour side / upper harbour flats",
    day: "Day 2",
    filterKey: "day2",
    ...
    operationalNote: "Chosen for the Monday to avoid the busier weekend conditions near the harbour edge. Wind was lighter closer to the harbour, then picked up a bit further down towards the bridge where the estuary opened out more.",
    deliverables: "High-resolution aerial image, surface height model, 3D ground model, cross-sections, panoramic views, future comparison views",
    weatherNotes: "Coordinate: 50.535818, -4.929569. Generally calmer closer to the harbour, with slightly more wind further down-estuary towards the bridge where the area felt more open.",
    surveyNotes: "This section was deliberately left for Monday so the harbour edge would be quieter than on the Sunday. Even so, the route avoided direct overflight of the busier public areas. Conditions were fairly calm near the harbour itself, with a bit more wind felt further down the estuary.",
    tags: ["Day 2", "Harbour side", "Harbour flats", "Post-low tide"],
    cardNote: "Harbour flats after the tide turn",
    purpose: "Shows the harbour-side flats as the water started to return."
  },
  area5: {
    ...
    tideScore: 87,
    missionRole: "Covers an important transition area between the central estuary and the outer estuary section.",
  ...
  area7: {
    ...
    operationalNote: "After the calm start further up-estuary, the wind picked up noticeably here. The drone was working harder and progress was slower when flying back towards the harbour side.",
    deliverables: "High-resolution aerial image, surface height model, 3D ground model, cross-sections, panoramic views, future comparison views",
    weatherNotes: "Coordinate: 50.532115, -4.887484. Wind was clearly stronger here than in A8, with airflow pushing down from the harbour direction and making this section more exposed.",
    surveyNotes: "This was the second main section flown on Day 2 after starting further up at Wadebridge. As the survey moved back down the estuary towards the harbour, the wind definitely increased and the aircraft had to work harder. It did not map as quickly into the wind, especially when tracking back towards the harbour edge, which was the direction the airflow seemed to be coming from.",
  ...
  area8: {
    ...
    operationalNote: "This was the furthest section from the harbour and the easiest way in and out was by bike with the kit on a trailer. Once there, conditions were calm and very peaceful with no real wind.",
    deliverables: "High-resolution aerial image, surface height model, 3D ground model, panoramic views, future comparison views",
...
const els = {
  ...
  fixedPointSelect: byId("fixedPointSelect"),
  ...
  fixedPointsSummary: byId("fixedPointsSummary"),
  fixedPointsHistory: byId("fixedPointsHistory"),
  fixedPointsMapImage: byId("fixedPointsMapImage"),
  fixedPointsMapMarkers: byId("fixedPointsMapMarkers"),
  fixedPointTitle: byId("fixedPointTitle"),
  fixedPointSummary: byId("fixedPointSummary"),
  fixedPointChips: byId("fixedPointChips"),
  fixedPointDetails: byId("fixedPointDetails"),
  fixedPointGallery: byId("fixedPointGallery"),
...
async function loadProjectDataset() {
  if (window.location.protocol === "file:") {
    ...
    throw new Error("This local preview needs the bundled project data script available beside `index.html` when opened directly.");
  }
...
function bindEvents() {
  ...
  els.fixedPointSelect?.addEventListener("change", () => {
    state.fixedPointId = els.fixedPointSelect.value;
    renderShellToolbar();
    renderFixedPoints();
    syncUrlState();
  });
  ...
  const handleVolumeLightboxTrigger = (event) => {
    ...
    openVolumeImageLightbox({
      eyebrow: trigger.dataset.volumeLightboxEyebrow || "Reference map",
      title: trigger.dataset.volumeLightboxTitle || "Reference image",
      caption: trigger.dataset.volumeLightboxCaption || "",
      src: trigger.dataset.volumeLightboxSrc || "",
      alt: trigger.dataset.volumeLightboxAlt || trigger.dataset.volumeLightboxTitle || "Reference image",
      legendItems: trigger.dataset.volumeLightboxLegend === "trend-map" ? state.volumeLightboxLegendItems : [],
      rotation: Number.parseFloat(trigger.dataset.volumeLightboxRotation || "0") || 0,
      secondarySrc: trigger.dataset.volumeLightboxSecondarySrc || "",
      secondaryAlt: trigger.dataset.volumeLightboxSecondaryAlt || "",
      primaryLabel: trigger.dataset.volumeLightboxPrimaryLabel || "",
      secondaryLabel: trigger.dataset.volumeLightboxSecondaryLabel || "",
      sharedHeading: trigger.dataset.volumeLightboxSharedHeading || ""
    });
  };
  ...
  els.fixedPointGallery?.addEventListener("click", handleVolumeLightboxTrigger);
  els.fixedPointsHistory?.addEventListener("click", handleVolumeLightboxTrigger);
...
function renderAll() {
  ...
  fillSelect(els.fixedPointSelect, fixedPointSelectOptions(), state.fixedPointId);
...
function renderShellToolbar() {
  ...
  const fixedPointField = document.querySelector(".shell-toolbar__field--fixedpoints");
  const fixedPointEnabled = state.activeTab === "fixedpoints" && hasFixedPointsLoaded();
  ...
  if (els.fixedPointSelect) {
    fillSelect(els.fixedPointSelect, fixedPointSelectOptions(), state.fixedPointId);
    els.fixedPointSelect.disabled = !fixedPointEnabled;
    els.fixedPointSelect.setAttribute("aria-disabled", String(!fixedPointEnabled));
  }

  document.querySelector(".shell-toolbar")?.classList.toggle("shell-toolbar--inactive", !(areaSelectEnabled || fixedPointEnabled));
  areaField?.classList.toggle("hidden", !areaSelectEnabled);
  fixedPointField?.classList.toggle("hidden", !fixedPointEnabled);
}
...
async function renderShellStage() {
  ...
  const overviewHeroTabs = new Set(["overview", "areas", "weather", "panorama", "fixedpoints"]);
  ...
  els.shellStageSurvey.innerHTML = `
    ...
    `;
  const showShellStageArea = areaHeroEnabled && configuredAreaHero && state.activeTab !== "panorama";
  ...
  `;
  els.shellStageArea.classList.toggle("hidden", !showShellStageArea);
...
function currentProjectFixedPoints() {
  return currentProjectContent()?.fixedPoints || null;
}

function hasFixedPointsLoaded() {
  return Array.isArray(currentProjectFixedPoints()?.points) && currentProjectFixedPoints().points.length > 0;
}

function fixedPointSelectOptions() {
  const points = currentProjectFixedPoints()?.points;
  if (!Array.isArray(points) || !points.length) {
    return [["", "No observation points loaded"]];
  }
  return points.map((point) => [point.id, `${point.number}. ${point.title}`]);
}
...
function activeTabShellMeta() {
  ...
  const metaByTab = {
    ...
    fixedpoints: {
      eyebrow: "Repeat Observation Network",
      title: "Fixed Observation Points",
      summary: "Use named estuary-wide observation points to line up repeat screenshots, checked heights, and before-and-after evidence against the same reference locations."
    },
    volume: {
...
async function renderOverview() {
  ...
  const isHayleProject = currentProject()?.id === "hayle-estuary";
  els.overviewAreaFilters.innerHTML = isHayleProject
    ? [
        `<button class="chip ${state.overviewAreaFilter === "all" ? "active" : ""}" type="button" data-overview-filter="all">All 3 areas</button>`,
        ...overviewAreas.map((area, index) => `<button class="chip ${state.overviewAreaFilter === area.id ? "active" : ""}" type="button" data-overview-filter="${escapeHtml(area.id)}">${escapeHtml(`A${index + 1}`)}</button>`)
      ].join("")
    : [
  ...
  els.overviewAreaGrid.style.gridTemplateColumns = isHayleProject
    ? `repeat(${Math.max(1, filteredOverviewAreas.length)}, minmax(0, 1fr))`
    : "";
  ...
  els.overviewAreaGrid.innerHTML = filteredOverviewAreas.map((area, index) => `
    <article class="card overview-area-card overview-area-card--${area.statusTone || overviewAreaTone(area.id, index)}">
      ...
      <div class="overview-area-card__stats">
        ...
        <div><span class="muted">${isHayleProject ? "Survey position" : "Low tide relationship"}</span><strong>${escapeHtml(area.launchOffset)}</strong></div>
        <div><span class="muted">${isHayleProject ? "Capture note" : "Tidal window"}</span><strong>${escapeHtml(area.tideWindow)}</strong></div>
      </div>
      ${isHayleProject ? `
      <div class="overview-area-timeline">
        <p class="overview-area-timeline__caption">${escapeHtml("This Hayle baseline was flown in smaller blocks across the day, so the timing is described in words rather than shown as one continuous low-tide position marker.")}</p>
      </div>
      ` : `
      <div class="overview-area-timeline">
        ...
        <p class="overview-area-timeline__caption">${escapeHtml("Marker based on mid-flight position in relation to the low-tide window.")}</p>
      </div>
      `}
...
function renderAreasLegacy() {
  ...
  els.selectedAreaAssets.innerHTML = `
    ...
    <article class="selected-score-card">
      <span class="selected-score-card__label">Flight pattern</span>
      <strong class="selected-score-card__value">${escapeHtml(area.actualDuration)}</strong>
      <span class="selected-score-card__sub">${escapeHtml(`Low tide reference ${area.lowTide} - ${area.lowTideHeight}`)}</span>
    </article>
    ` : `
    <article class="selected-score-card">
    ...
    <article class="selected-detail-panel">
      ...
      <div class="selected-detail-table">
        ...
        ${isHayleProject ? selectedDetailRow("Timing note", "Flights were broken into smaller blocks, so this area does not map cleanly to one single tide-position marker.") : selectedDetailRow("Mid-flight tide marker", midOffsetLabel(area.midOffsetMinutes))}
      </div>
    </article>
    ${surveyComparisonMarkup(area.id)}
    <article class="selected-detail-panel">
...
async function renderSections() {
  ...
  let area = applySharedSectionGeometry(baseArea, geometryMap);
  const previousSectionId = state.sectionId;
  const requestedSectionId = state.sectionId || area.sections[0]?.id;
  ...
  const [surveyRows, basemapPath, overlayPath] = await Promise.all([
    Promise.all(project.surveys.map(async (profileSurvey) => ({
      ...
      rows: await loadSectionRowsForSurvey(project.id, profileSurvey, area.id, requestedSectionId)
    }))),
    resolveExistingAsset(basemapCandidates),
    resolveExistingAsset(overlayCandidates)
  ]);
  const areaHasTrackGeometry = area.sections.every((item) => (
    Number.isFinite(item?.track?.start?.x)
    && Number.isFinite(item?.track?.start?.y)
    && Number.isFinite(item?.track?.end?.x)
    && Number.isFinite(item?.track?.end?.y)
  ));
  if (!geometryMap?.size && !areaHasTrackGeometry && shouldUseDetectedSectionTracks(project.id, area.id)) {
    const svgPath = await resolveExistingAsset([
      projectAssetPath(project.id, "maps", area.id, "section-lines.svg"),
      projectAssetPath(project.id, "reports", area.id, "section-lines.svg"),
      projectAssetPath(project.id, "stats", area.id, "section-lines.svg"),
      sharedAreaAssetPath(project.id, area.id, "section-lines.svg")
    ]);
    const svgTracks = svgPath ? await loadSectionTracksFromSvg(svgPath) : [];
    if (svgTracks.length === area.sections.length) {
      area = applyDetectedSectionTracks(area, svgTracks);
    } else if (overlayPath) {
      const detectedTracks = await detectSectionTracksFromImage(overlayPath, area.sections.length);
      area = applyDetectedSectionTracks(area, detectedTracks);
    }
  }
...
function renderWeather() {
  ...
  const dashboardUrl = new URL("/weather-dashboard.html", window.location.origin);
  dashboardUrl.search = params.toString();
  const src = dashboardUrl.toString();
...
function renderFixedPoints() {
  if (!els.fixedPointsSummary || !els.fixedPointDetails) {
    return;
  }

  const config = currentProjectFixedPoints();
  const points = Array.isArray(config?.points) ? config.points : [];
  if (!config || !points.length) {
    els.fixedPointsSummary.textContent = "No fixed-point package has been loaded for this project yet.";
    els.fixedPointsHistory.innerHTML = "";
    els.fixedPointsMapImage.removeAttribute("src");
    els.fixedPointsMapImage.alt = "";
    els.fixedPointsMapMarkers.innerHTML = "";
    els.fixedPointTitle.textContent = "Observation Point";
    els.fixedPointSummary.textContent = "Load fixed-point assets to review repeat screenshots and checked heights here.";
    els.fixedPointChips.innerHTML = "";
    els.fixedPointDetails.innerHTML = "";
    els.fixedPointGallery.innerHTML = "";
    if (els.fixedPointSelect) {
      fillSelect(els.fixedPointSelect, fixedPointSelectOptions(), "");
      els.fixedPointSelect.disabled = true;
      els.fixedPointSelect.setAttribute("aria-disabled", "true");
    }
    return;
  }

  const selectedPoint = points.find((point) => point.id === state.fixedPointId) || points[0];
  state.fixedPointId = selectedPoint.id;
  if (els.fixedPointSelect) {
    fillSelect(els.fixedPointSelect, fixedPointSelectOptions(), state.fixedPointId);
    els.fixedPointSelect.disabled = state.activeTab !== "fixedpoints";
    els.fixedPointSelect.setAttribute("aria-disabled", String(state.activeTab !== "fixedpoints"));
  }

  els.fixedPointsSummary.textContent = config.summary || "Fixed observation points are available for this project.";
  els.fixedPointsMapImage.src = config.mapImagePath || "";
  els.fixedPointsMapImage.alt = config.mapImageAlt || "Fixed point location map";

  const referenceImages = Array.isArray(config.referenceImages) ? config.referenceImages : [];
  const referenceDocuments = Array.isArray(config.referenceDocuments) ? config.referenceDocuments : [];
  els.fixedPointsHistory.innerHTML = (referenceImages.length || referenceDocuments.length) ? `
    <article class="fixed-points-history__panel">
      <div class="fixed-points-history__header">
        <div>
          <p class="eyebrow">Previous Surveys</p>
          <h3>Historic Survey Context</h3>
          <p class="subtext muted">Use these earlier bathymetry surfaces alongside the January 2026 observation points to compare how the monitored ground has shifted over time.</p>
        </div>
      </div>
      <div class="fixed-points-history__grid">
        ${referenceImages.map((image) => `
          <article class="fixed-points-history__card">
            <button
              class="fixed-points-history__trigger"
              type="button"
              data-volume-lightbox-src="${escapeAttr(image.src)}"
              data-volume-lightbox-eyebrow="${escapeAttr(image.eyebrow || "Historic bathymetry")}"
              data-volume-lightbox-title="${escapeAttr(image.title || "Historic survey context")}"
              data-volume-lightbox-caption="${escapeAttr(image.caption || "")}"
              data-volume-lightbox-alt="${escapeAttr(image.alt || image.title || "Historic survey image")}"
            >
              <img src="${escapeAttr(image.src)}" alt="${escapeAttr(image.alt || image.title || "Historic survey image")}" loading="lazy">
              <span class="fixed-points-history__expand">Open full screen</span>
            </button>
            <div class="fixed-points-history__meta">
              <span class="fixed-points-history__badge">${escapeHtml(image.badge || image.label || "Historic survey")}</span>
              <h4>${escapeHtml(image.title || image.label || "Historic survey image")}</h4>
              <p>${escapeHtml(image.summary || image.caption || "Historic context image.")}</p>
            </div>
          </article>
        `).join("")}
      </div>
      ${referenceDocuments.length ? `
        <div class="fixed-points-history__docs">
          ${referenceDocuments.map((item) => `
            <a class="fixed-points-doc" href="${escapeAttr(item.path)}" target="_blank" rel="noopener noreferrer">
              <strong>${escapeHtml(item.label)}</strong>
              <span>${escapeHtml(item.description || "Reference document")}</span>
            </a>
          `).join("")}
        </div>
      ` : ""}
  ...
  els.fixedPointsMapMarkers.innerHTML = points.map((point) => `
    <button
      class="fixed-points-marker ${point.id === selectedPoint.id ? "is-active" : ""}"
      type="button"
      data-fixed-point="${escapeAttr(point.id)}"
      style="left:${Number(point.map?.x) || 0}%; top:${Number(point.map?.y) || 0}%;"
      aria-label="${escapeAttr(`${point.number}. ${point.title}`)}"
      title="${escapeAttr(`${point.number}. ${point.title}`)}"
    >
      ${escapeHtml(String(point.number || ""))}
    </button>
  `).join("");

  const pointImages = Array.isArray(selectedPoint.images) ? selectedPoint.images : [];
  const modelImages = Array.isArray(selectedPoint.modelImages) ? selectedPoint.modelImages : [];
  const comparisonPairs = Array.from(
    { length: Math.max(pointImages.length, modelImages.length) },
    (_, index) => ({
      reference: pointImages[index] || null,
      model: modelImages[index] || null,
      slotLabel: index === 0 ? "Reference A" : index === 1 ? "Reference B" : `Reference ${index + 1}`
    })
  ).filter((pair) => pair.reference || pair.model);
  els.fixedPointTitle.textContent = `${selectedPoint.number}. ${selectedPoint.title}`;
  els.fixedPointSummary.textContent = selectedPoint.summary || "Fixed-point summary pending.";
  els.fixedPointChips.innerHTML = [
    selectedPoint.areaHint ? `<span class="chip">${escapeHtml(selectedPoint.areaHint)}</span>` : "",
    `<span class="chip">17 Jul 2026 baseline ready</span>`,
    `<span class="chip">21 Jan 2026 reference loaded</span>`,
    `<span class="chip">${modelImages.length ? "Current matched capture loaded" : "Current matched capture pending"}</span>`
  ].filter(Boolean).join("");

  els.fixedPointDetails.innerHTML = [
    detail("Why this point matters", selectedPoint.whyItMatters || "Repeat observation point."),
    detail("Current workflow", "Use the January 21 2026 photo set as the historic reference, then match the same point in the current DJI Terra or Nira model."),
    detail("Next capture", selectedPoint.nextCapture || "Add a current aligned screenshot and checked height for this point."),
    detail("Reference documents", "Use the supplied 2021 and December 2024 bathymetry files as supporting evidence alongside the fixed-point images.")
  ].join("");

  els.fixedPointGallery.innerHTML = comparisonPairs.length ? comparisonPairs.map((pair) => `
    <article class="fixed-points-comparison">
      <div class="fixed-points-comparison__head">
        <div>
          <p class="eyebrow">${escapeHtml(pair.slotLabel)}</p>
          <h4>${escapeHtml(`${selectedPoint.number}. ${selectedPoint.title}`)}</h4>
        </div>
        ${pair.reference && pair.model ? `
          <button
            class="chip fixed-points-comparison__action"
            type="button"
            data-volume-lightbox-src="${escapeAttr(pair.reference.src)}"
            data-volume-lightbox-secondary-src="${escapeAttr(pair.model.src)}"
            data-volume-lightbox-eyebrow="Fixed observation comparison"
            data-volume-lightbox-title="${escapeAttr(`${selectedPoint.number}. ${selectedPoint.title} (${pair.slotLabel})`)}"
            data-volume-lightbox-caption="${escapeAttr(`${pair.reference.label || "Historic reference"} beside ${pair.model.label || "DJI Terra matched view"}`)}"
            data-volume-lightbox-alt="${escapeAttr(pair.reference.label || selectedPoint.title)}"
            data-volume-lightbox-secondary-alt="${escapeAttr(pair.model.label || selectedPoint.title)}"
            data-volume-lightbox-primary-label="${escapeAttr(pair.reference.label || "January 2026 reference")}"
            data-volume-lightbox-secondary-label="${escapeAttr(pair.model.label || "DJI Terra matched view")}"
          >
            Compare full screen
          </button>
        ` : ""}
      </div>
      <div class="fixed-points-comparison__grid">
        ${pair.reference ? `
          <figure class="fixed-points-gallery__item fixed-points-gallery__item--comparison">
            <figcaption class="fixed-points-gallery__label">${escapeHtml(pair.reference.label || "Reference image")}</figcaption>
            <button
              class="fixed-points-gallery__trigger"
              type="button"
              data-volume-lightbox-src="${escapeAttr(pair.reference.src)}"
              data-volume-lightbox-eyebrow="Observation point reference"
              data-volume-lightbox-title="${escapeAttr(`${selectedPoint.number}. ${selectedPoint.title}`)}"
              data-volume-lightbox-caption="${escapeAttr(pair.reference.label || "Historic reference image")}"
              data-volume-lightbox-alt="${escapeAttr(pair.reference.label || selectedPoint.title)}"
            >
              <img src="${escapeAttr(pair.reference.src)}" alt="${escapeAttr(pair.reference.label || selectedPoint.title)}" loading="lazy">
              <span class="fixed-points-gallery__expand">Open full screen</span>
            </button>
          </figure>
        ` : `
          <article class="fixed-points-gallery__placeholder">
            <p class="eyebrow">January 2026</p>
            <h4>Reference image missing</h4>
            <p class="muted">Drop the historic reference image in here to complete this comparison pair.</p>
          </article>
        `}
        ${pair.model ? `
          <figure class="fixed-points-gallery__item fixed-points-gallery__item--comparison">
            <figcaption class="fixed-points-gallery__label">${escapeHtml(pair.model.label || "Current model capture")}</figcaption>
            <button
              class="fixed-points-gallery__trigger"
              type="button"
              data-volume-lightbox-src="${escapeAttr(pair.model.src)}"
              data-volume-lightbox-eyebrow="Current model capture"
              data-volume-lightbox-title="${escapeAttr(`${selectedPoint.number}. ${selectedPoint.title}`)}"
              data-volume-lightbox-caption="${escapeAttr(pair.model.label || "Current matched screenshot")}"
              data-volume-lightbox-alt="${escapeAttr(pair.model.label || selectedPoint.title)}"
            >
              <img src="${escapeAttr(pair.model.src)}" alt="${escapeAttr(pair.model.label || selectedPoint.title)}" loading="lazy">
              <span class="fixed-points-gallery__expand">Open full screen</span>
            </button>
          </figure>
        ` : `
          <article class="fixed-points-gallery__placeholder">
            <p class="eyebrow">DJI Terra</p>
            <h4>Matched view missing</h4>
            <p class="muted">Drop the aligned DJI Terra screenshot in here to complete this comparison pair.</p>
          </article>
        `}
      </div>
    </article>
  `).join("") : `
    <article class="fixed-points-gallery__placeholder">
      <p class="eyebrow">Observation Comparison</p>
      <h4>Awaiting reference imagery</h4>
      <p class="muted">Load the historic and current observation images for this point to start side-by-side comparison.</p>
    </article>
  `;

  document.querySelectorAll("[data-fixed-point]").forEach((button) => {
    button.addEventListener("click", () => {
      state.fixedPointId = button.dataset.fixedPoint;
      renderShellToolbar();
      renderFixedPoints();
    });
  });
...
async function loadSectionRowsForSurvey(projectId, survey, areaId, sectionId) {
  ...
  return normaliseSectionRows(rows, {
    reverseDirection: sectionProfileShouldBeReversed(projectId, areaId, sectionId)
  });
}

function normaliseSectionRows(rows, options = {}) {
  const normalised = rows
    ...
    .map((row) => ({
      ...
      clippedLow: row.height < SECTION_DISPLAY_MIN_HEIGHT,
      clippedHigh: row.height > SECTION_DISPLAY_MAX_HEIGHT,
      height: clamp(row.height, SECTION_DISPLAY_MIN_HEIGHT, SECTION_DISPLAY_MAX_HEIGHT)
    }));

  if (!options.reverseDirection || normalised.length < 2) {
    return normalised;
  }

  const minDistance = normalised[0].distance;
  const maxDistance = normalised[normalised.length - 1].distance;
  return normalised
    .map((row) => ({
      ...row,
      distance: maxDistance - (row.distance - minDistance)
    }))
    .sort((a, b) => a.distance - b.distance);
}
...
function sectionComparisonWarnings(orderedProfiles) {
  ...
  const hasCappedHighSegment = orderedProfiles.some((profile) => profile.heightAtFocus >= 4.99);
  ...
  if (hasCappedHighSegment) {
    warnings.push("Long-dash stretches at +5 m are visually capped for readability, so the real ground there may continue higher.");
  }
...
function drawSectionChart(profiles, section, anchorSurveyId = state.surveyId) {
  ...
  const profileSeries = activeProfiles.map((profile) => {
    ...
    return {
      ...
      clippedHighSegments: buildSegments((point) => point.row.height !== 0 && point.row.clippedHigh),
      clippedLowSegments: buildSegments((point) => point.row.height !== 0 && point.row.clippedLow),
  ...
  const profileLines = profileSeries.map((profile) => {
    ...
    const measuredLines = profile.measuredSegments
      ...
      .join("");
    const clippedLineMarkup = (segment, dasharray) => {
      if (segment.length === 1) {
        const point = segment[0];
        return `
          <circle cx="${fixed(point.x, 2)}" cy="${fixed(point.y, 2)}" r="3.2" fill="${profile.color}" opacity="0.98"/>
          <circle cx="${fixed(point.x, 2)}" cy="${fixed(point.y, 2)}" r="5.4" fill="none" stroke="rgba(255,255,255,0.9)" stroke-width="1.4" stroke-dasharray="${dasharray}"/>
        `;
      }
      return `
        <polyline fill="none" stroke="${profile.color}" stroke-width="${fixed(strokeWidth + 1, 2)}" stroke-linecap="round" stroke-linejoin="round" opacity="0.96" points="${buildPointString(segment)}"/>
        <polyline fill="none" stroke="rgba(255,255,255,0.88)" stroke-width="${fixed(Math.max(1.05, strokeWidth * 0.7), 2)}" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="${dasharray}" points="${buildPointString(segment)}"/>
      `;
    };
    const clippedHighLines = profile.clippedHighSegments
      .map((segment) => clippedLineMarkup(segment, "11 6"))
      .join("");
    const clippedLowLines = profile.clippedLowSegments
      .map((segment) => clippedLineMarkup(segment, "3 8"))
      .join("");
    ...
    const hoverDot = profile.hoverRow && hoverDistance !== null
      ...
      : "";
    return `${zeroLines}${measuredLines}${clippedHighLines}${clippedLowLines}${hoverDot}`;
  }).join("");
...
function buildAreaLayers(areaLabel) {
  ...
  function buildArea(definition, index = 0) {
    ...
    const explicitTracks = explicitSectionImageTracks(currentProject()?.id, key);
...
function polygonLabelPoint(points) {
  ...
  async function loadSharedSectionGeometry(projectId, areaId) {
    ...
    const explicitTracks = explicitSectionImageTracks(projectId, areaId);
...
async function detectSectionTracksFromImage(imagePath, expectedCount) {
  ...
  const tracks = await new Promise((resolve) => {
    ...
    image.onload = () => {
      ...
      if (!expectedCount || !mask.some(Boolean)) {
        resolve([]);
        return;
      }
      ...
      const neighbourOffsets = [
        -targetWidth - 1, -targetWidth, -targetWidth + 1,
        -1, 1,
        targetWidth - 1, targetWidth, targetWidth + 1
      ];
      ...
      for (let y = 0; y < targetHeight; y += 1) {
        for (let x = 0; x < targetWidth; x += 1) {
          const startIndex = (y * targetWidth) + x;
          ...
          while (stack.length) {
            const index = stack.pop();
            const pointX = index % targetWidth;
            const pointY = Math.floor(index / targetWidth);
            component.push({ x: pointX, y: pointY });

            for (const offset of neighbourOffsets) {
              const nextIndex = index + offset;
              if (nextIndex < 0 || nextIndex >= mask.length || visited[nextIndex] || !mask[nextIndex]) {
                continue;
              }
              const nextX = nextIndex % targetWidth;
              const nextY = Math.floor(nextIndex / targetWidth);
              if (Math.abs(nextX - pointX) > 1 || Math.abs(nextY - pointY) > 1) {
                continue;
              }
              visited[nextIndex] = 1;
      ...
      const extracted = components
        .sort((left, right) => right.length - left.length)
        .slice(0, expectedCount)
        .map((component) => {
          const meanX = component.reduce((sum, point) => sum + point.x, 0) / component.length;
          const meanY = component.reduce((sum, point) => sum + point.y, 0) / component.length;
          let xx = 0;
          let yy = 0;
          let xy = 0;
          ...
          const projected = component.map((point) => ({
            point,
            t: ((point.x - meanX) * axisX) + ((point.y - meanY) * axisY)
          }));
          const minT = Math.min(...projected.map((entry) => entry.t));
          const maxT = Math.max(...projected.map((entry) => entry.t));
          const spanT = Math.max(1, maxT - minT);
          const bucketCount = Math.max(10, Math.min(28, Math.round(component.length / 35)));
          const buckets = Array.from({ length: bucketCount }, () => []);
          projected.forEach((entry) => {
            const ratio = clamp((entry.t - minT) / spanT, 0, 0.999999);
            const bucketIndex = Math.min(bucketCount - 1, Math.floor(ratio * bucketCount));
            buckets[bucketIndex].push(entry.point);
          });

          const points = buckets
            .filter((bucket) => bucket.length)
            .map((bucket) => ({
              x: (bucket.reduce((sum, point) => sum + point.x, 0) / bucket.length / targetWidth) * 100,
              y: (bucket.reduce((sum, point) => sum + point.y, 0) / bucket.length / targetHeight) * 100
            }));
        ...
        .filter((track) => Array.isArray(track.track?.points) && track.track.points.length >= 2);
...
async function loadSectionTracksFromSvg(svgPath) {
  if (state.sectionTrackCache.has(svgPath)) {
    return state.sectionTrackCache.get(svgPath);
  }

  const tracks = await fetch(svgPath)
    ...
    .then((markup) => {
      ...
      return Array.from(documentNode.querySelectorAll("line"))
        .map((line) => {
        ...
        .filter((track) => track && track.track?.points?.length === 2);
    })
    .catch(() => []);

  state.sectionTrackCache.set(svgPath, tracks);
  return tracks;
}
...
function applyDetectedSectionTracks(area, detectedTracks) {
  if (!Array.isArray(detectedTracks) || detectedTracks.length !== area.sections.length) {
    return area;
  }

  const sortedTracks = [...detectedTracks].sort((left, right) => left.hotspot.x - right.hotspot.x);
  return {
    ...area,
    sections: area.sections.map((section, index) => {
      const detected = sortedTracks[index];
      return detected ? { ...section, ...detected } : section;
    })
  };
}
...
function updateViewerStageSizing() {
  if (!els.viewerStage) {
    return;
  }
  ...
  if (!image) {
    els.viewerStage.style.removeProperty("--viewer-image-ratio");
    return;
  }

  els.viewerStage.style.setProperty("--viewer-image-ratio", `${image.naturalWidth} / ${image.naturalHeight}`);
}

function applyViewTransform() {
  updateViewerStageSizing();
  clampPanToBounds();
...
async function toggleAdminMode() {
  ...
  const response = await fetch("/api/admin-auth", requestOptions);
  const payload = await response.json().catch(() => ({}));
...
function openVolumeImageLightbox(arg1, title, caption, src, alt, legendItems = [], rotation = 0) {
  const config = typeof arg1 === "object" && arg1 !== null
    ? arg1
    : { eyebrow: arg1, title, caption, src, alt, legendItems, rotation };
  if (!config.src || !els.volumeImageLightbox) {
    return;
  }
  els.volumeImageLightboxEyebrow.textContent = config.eyebrow || "Reference map";
  els.volumeImageLightboxTitle.textContent = config.title || "Reference image";
  els.volumeImageLightboxCaption.textContent = config.caption || "";
  els.volumeImageLightboxImage.src = config.src;
  els.volumeImageLightboxImage.alt = config.alt || config.title || "Reference image";
  const hasSecondaryImage = Boolean(config.secondarySrc);
  const sharedHeading = config.sharedHeading || "";
  els.volumeImageLightboxSecondaryPane?.classList.toggle("hidden", !hasSecondaryImage);
  if (els.volumeImageLightboxSecondaryImage) {
    if (hasSecondaryImage) {
      els.volumeImageLightboxSecondaryImage.src = config.secondarySrc;
      els.volumeImageLightboxSecondaryImage.alt = config.secondaryAlt || config.title || "Reference image";
    } else {
      els.volumeImageLightboxSecondaryImage.removeAttribute("src");
      els.volumeImageLightboxSecondaryImage.alt = "";
    }
  }
  const primaryPaneLabel = config.primaryLabel || "";
  const secondaryPaneLabel = hasSecondaryImage ? (config.secondaryLabel || "") : "";
  els.volumeImageLightboxPrimaryLabel?.classList.toggle("hidden", !primaryPaneLabel);
  if (els.volumeImageLightboxPrimaryLabel) {
    els.volumeImageLightboxPrimaryLabel.textContent = primaryPaneLabel;
  }
  els.volumeImageLightboxSecondaryLabel?.classList.toggle("hidden", !(hasSecondaryImage && secondaryPaneLabel));
  if (els.volumeImageLightboxSecondaryLabel) {
    els.volumeImageLightboxSecondaryLabel.textContent = hasSecondaryImage ? secondaryPaneLabel : "";
  }
  els.volumeImageLightboxPrimaryOverlay?.classList.add("hidden");
  if (els.volumeImageLightboxPrimaryOverlay) {
    els.volumeImageLightboxPrimaryOverlay.textContent = "";
  }
  els.volumeImageLightboxSecondaryOverlay?.classList.toggle("hidden", !(hasSecondaryImage && sharedHeading));
  if (els.volumeImageLightboxSecondaryOverlay) {
    els.volumeImageLightboxSecondaryOverlay.textContent = hasSecondaryImage ? sharedHeading : "";
  }
  els.volumeImageLightboxCanvas?.classList.toggle("is-comparison", hasSecondaryImage);
  state.volumeLightboxRotation = config.rotation || 0;
  renderVolumeImageLightboxLegend(config.legendItems || []);
...
function closeVolumeImageLightbox() {
  ...
  els.volumeImageLightboxSecondaryImage?.removeAttribute("src");
  if (els.volumeImageLightboxPrimaryLabel) {
    els.volumeImageLightboxPrimaryLabel.textContent = "";
    els.volumeImageLightboxPrimaryLabel.classList.add("hidden");
  }
  if (els.volumeImageLightboxSecondaryLabel) {
    els.volumeImageLightboxSecondaryLabel.textContent = "";
...
function clampVolumeImageLightboxPan() {
  if (!els.volumeImageLightboxCanvas || !els.volumeImageLightboxViewport) {
    return;
  }
  ...
  const imageWidth = els.volumeImageLightboxCanvas.offsetWidth;
  const imageHeight = els.volumeImageLightboxCanvas.offsetHeight;
...
function applyVolumeImageLightboxTransform() {
  if (!els.volumeImageLightboxCanvas) {
    return;
  }
  els.volumeImageLightboxCanvas.style.transform = `${state.volumeLightboxRotation ? `rotate(${state.volumeLightboxRotation}deg) ` : ""}translate(${state.volumeLightboxPanX}px, ${state.volumeLightboxPanY}px) scale(${state.volumeLightboxZoom})`.trim();
  els.volumeImageLightboxCanvas.style.transformOrigin = "center center";
...
function renderVolumeImageLightboxLegend(items = []) {
  ...
  els.volumeImageLightboxContent?.classList.add("has-legend");
  els.volumeImageLightboxLegend.innerHTML = `
